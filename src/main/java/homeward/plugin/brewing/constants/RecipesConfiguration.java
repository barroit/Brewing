package homeward.plugin.brewing.constants;

public class RecipesConfiguration {
    public static final String FILE_NAME = "recipes.yml";

    public static final String DISPLAY_NAME = "display-name";

    public static final String SUBSTRATE = "substrate";
    public static final String RESTRICTION = "restriction";
    public static final String YEAST = "yeast";
    public static final String OUTPUT = "output";

    public static final String YIELD = "yield";
    public static final String MAX = "max";
    public static final String MIN = "min";

    public static final String INDEX = "index";
    public static final String RESTRICTION_INDEX = "restriction-index";
    public static final String YEAST_INDEX = "yeast-index";
    public static final String YIELD_INDEX = "yield-index";

    public static final String BREWING_CYCLE = "brewing-cycle";

    public static final String VANILLA = "Vanilla";
    public static final String ITEMSADDER = "ItemsAdder";
    public static final String MMOITEMS = "MMOItems";

    public static final String PROVIDER = "provider";
    public static final String ITEM = "item";

    public static final String SCALED = "scaled";
    public static final String TIER = "tier";
    public static final String TYPE = "type";
    public static final String ID = "id";
    public static final String NAMESPACE = "namespace";
    public static final String MATERIAL = "material";
    public static final String POTION_TYPE = "potion-type";
    public static final String NAMESPACED_ID = "namespacedId";
    public static final String QUANTITY = "quantity";
}
